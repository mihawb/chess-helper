...
def detect_api(input_file,output_file):
	global NC_LAYER, NC_IMAGE, NC_CONFIG
	utils.reset()
	if (not os.path.isfile(input_file)):
		utils.errn("error: the file \"%s\" does not exits" % input_file)

	NC_IMAGE, NC_LAYER = ImageObject(load(input_file)), 0
	for _ in range(NC_CONFIG['layers']):
		NC_LAYER += 1; layer()
	save(output_file, NC_IMAGE['orig'])
	K.clear_session(); gc.collect()	
	print("DETECT: %s" % input_file)
